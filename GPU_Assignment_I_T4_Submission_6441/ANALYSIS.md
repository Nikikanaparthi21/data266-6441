...
However, the original loop performed one short matmul and then slept for five seconds. Sampled
utilization was therefore mostly near zero, the SM clock remained around
585 MHz, and temperature remained near
49 C. This is an intermittent-load diagnostic, not a valid
sustained thermal stress test. The result must not be presented as proof of full-load thermal
behavior.

## Part F - Overall findings

The measurements demonstrate the difference between compute-oriented matmul and a memory-bound
elementwise operation, and they directly confirm quadratic memory growth for naive attention.
Fused attention substantially reduces memory and improves latency. The central limitations are
the non-RTX T4 device, unsupported native TF32/BF16 comparisons, absence of an observed OOM
failure, and the intermittent thermal workload. These limitations are stated rather than hidden
or replaced with invented results.
